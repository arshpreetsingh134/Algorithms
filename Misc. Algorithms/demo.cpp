...
float ClosestPair(vector<Point> &Px, vector<Point> &Py, int n)
{
    ...
    float d_split = ClosestSplitPair(Sy, delta);

    if (d_split == 0)
        return delta;
    else
        return (d_split > delta) ? delta : d_split;

    //return min(delta, d_split);
}

// The main function that finds the smallest distance,
// integration all the functions into one
...
int main()
{
    vector<Point> points{{36, 69}, {68, 50}, {-50, -2}, {22, 18}, {-3, -4}, {58, -23}, {-24, 32}};
